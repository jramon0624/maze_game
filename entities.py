...
class MazeGenerator():
    ...
    def set_goal(self):
        if not self.position in self.goal_list:
            self.goal_list.append(self.position.copy())
        self.goal = rd.choice(self.goal_list)
...
class AgentT3(MazeGenerator):
    ...
    def move(self,maze,goal,*args):
        ...
        if not self.next_path:
            ...
            if not any([goal in this_path for this_path, _ in path_to_delete]):
                ...
                for path, _ in path_to_delete:
                    if goal in path:
                        goal_path = path

            if goal_path:
                self.next_path = [goal_path]
        else:
            self.tour.append(self.position)
            self.visited.append(self.position)
